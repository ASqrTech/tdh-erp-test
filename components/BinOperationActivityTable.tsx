...
import { BinOperationDetailsModal } from './BinOperationDetailsModal';

type TimeFilter = '24h' | 'week' | 'month' | 'custom';

export const BinOperationActivityTable: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    const { logs } = useAuth();
    ...
    const driverNameMap = useMemo(() => {
        ...
        logs.forEach(log => {
            if (typeof log.details === 'object' && log.details?.stageId === 'arrival' && log.details.submittedData) {
                const data = log.details.submittedData;
                if (data.vehicle_number && data.driver_name) {
                    map.set(data.vehicle_number, data.driver_name);
                }
            }
        });
        return map;
    ...
    const filteredLogs = useMemo(() => {
        ...
        const binLogs = logs
            .filter(log =>
                ...
                log.details.stageId === 'bin_operation'
            )
            .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());

        return binLogs.filter(log => {
            // Time Filter
    ...
    }, [logs, currentUser.id, searchTerm, timeFilter, driverNameMap, customStartDate, customEndDate]);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md w-full">
            <h3 className="text-xl font-semibold text-slate-700 mb-4">Bin Operation Activity Log</h3>
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 text-xs text-slate-500 uppercase">
                        <tr>
                            ...
                            <th className="p-3">Bin Status</th>
                    ...
                    <tbody>
                        {filteredLogs.length > 0 ? filteredLogs.map(log => {
                            const data = (log.details as any).submittedData;
                            const driverName = driverNameMap.get(data.vehicle_number) || <span className="text-xs text-slate-400">N/A</span>;
                            return (
                                <tr key={log.id} className="border-b hover:bg-slate-50">
                                    ...
                                    <td className="p-3">
                                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                                            data.bin_status === 'Fill' ? 'bg-green-100 text-green-800' :
                                            data.bin_status === 'Discharge' ? 'bg-red-100 text-red-800' :
