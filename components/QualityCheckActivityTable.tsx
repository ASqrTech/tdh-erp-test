...
export const QualityCheckActivityTable: React.FC<{ currentUser: User }> = ({ currentUser }) => {
    ...
    const filteredLogs = useMemo(() => {
        ...
        return qualityLogs.filter(log => {
            ...
            return (
                ...
                data.transaction_id?.toLowerCase().includes(lowercasedSearch) ||
                driverName.toLowerCase().includes(lowercasedSearch)
            );
        });

    }, [logs, searchTerm, timeFilter, driverNameMap, customStartDate, customEndDate]);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md w-full">
            <h3 className="text-xl font-semibold text-slate-700 mb-4">Quality Check Activity Log</h3>
                        ...
                        {filteredLogs.length > 0 ? filteredLogs.map(log => {
                            ...
                            return (
                                <tr key={log.id} className="border-b hover:bg-slate-50">
                                    ...
                                    <td className="p-3">{data.moisture_content_percent}</td>
                                    <td className="p-3">
                                        <button 
