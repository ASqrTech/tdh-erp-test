...
export const QualityCheckDetailsModal: React.FC<QualityCheckDetailsModalProps> = ({ record, onClose }) => {
    ...
    const getStatusBadge = (status: string) => {
        switch (status?.toUpperCase()) {
            case 'APPROVED':
                return <span className="text-2xl font-extrabold text-green-600">APPROVED</span>;
            case 'REJECTED':
                return <span className="text-2xl font-extrabold text-red-600">REJECTED</span>;
            default:
                return <span className="text-2xl font-extrabold text-yellow-600">PENDING</span>;
        }
    };
                ...
                <div className="p-6 border-b flex justify-between items-center bg-white rounded-t-2xl">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800">
                            Quality Inspection: {getStatusBadge(details.quality_status)}
                        </h2>
                        <p className="text-sm text-slate-500">
                            Vehicle: <span className="font-semibold">{details.vehicle_number}</span> | Ticket: <span className="font-semibold">{details.ticket_no}</span>
                        </p>
                    </div>
                ...
                <div className="p-8 max-h-[70vh] overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white p-6 rounded-xl shadow-md space-y-4">
                            <h3 className="col-span-2 text-xl font-bold text-red-700 border-b-2 border-red-200 pb-2 mb-2">Test Results</h3>
                            <DetailItem label="Sample ID" value={details.sample_id} />
                            <DetailItem label="Moisture Content (%)" value={details.moisture_content} />
                            <DetailItem label="Visual Inspection Notes" value={details.visual_inspection_notes} />
                        </div>

                        <div className="bg-white p-6 rounded-xl shadow-md space-y-4">
                            <h3 className="text-xl font-bold text-red-700 border-b-2 border-red-200 pb-2 mb-2">Inspector Details</h3>
                            <DetailItem label="Inspected By" value={record.userName} />
                            <DetailItem label="Inspection Time" value={new Date(record.timestamp).toLocaleString('en-US', { dateStyle: 'full', timeStyle: 'short' })} />
                        </div>
                    </div>

                     {details.remarks && (
                        <div className="mt-8">
                            <h3 className="text-xl font-bold text-red-700 border-b-2 border-red-200 pb-2 mb-2">Inspector Remarks</h3>
                            <p className="text-base text-slate-700 bg-white p-4 rounded-xl shadow-md">{details.remarks}</p>
                        </div>
                    )}
                </div>
